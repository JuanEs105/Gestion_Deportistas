...
async function enviarCodigoActivacion(email) {
    try {
        ...
        const response = await fetch('http://localhost:5000/api/auth/solicitar-codigo-registro', {
            method: 'POST',
            headers: {
        ...
        if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
            ...
            console.warn('⚠️ Asegúrate de que el backend esté corriendo en http://localhost:5000');
            
            // Verificar si el backend está activo
            try {
                const healthCheck = await fetch('http://localhost:5000/api/auth/health', { 
                    method: 'GET',
                ...
                throw new Error('Backend no disponible. Asegúrate de que el servidor esté corriendo en http://localhost:5000');
            }
