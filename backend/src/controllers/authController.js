...
class AuthController {
  ...
  try {
    ...
    // Buscar usuario CON niveles_asignados
    console.log('🔍 Buscando usuario en BD...');
    const user = await User.findOne({
      where: { email },
      attributes: ['id', 'nombre', 'email', 'password', 'role', 'activo', 'telefono', 'niveles_asignados'], // ← AGREGAR niveles_asignados
      include: [{
        model: Deportista,
        as: 'deportista',
        required: false,
    ...
    console.log('📚 Niveles asignados:', user.niveles_asignados);
    ...
    const token = jwt.sign(
      { 
        ...
        email: user.email, 
        role: user.role,
        niveles_asignados: user.niveles_asignados // ← INCLUIR en el token
      },
    ...
    console.log('📦 Datos enviados:', userResponse);
